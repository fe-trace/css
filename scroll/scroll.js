...
*/
function ScrollBar(){this.init.apply(this,arguments)}
ScrollBar.prototype={
	...
	resetLayout:function(){
		var opt=this.opt;
		this.maxH=this.scrollBar.clientHeight-this.scrollUp.clientHeight+opt.scrollDValue/2-this.scrollHandle.clientHeight;
		this.minH=this.scrollUp.clientHeight-opt.scrollDValue/2;
		//重置滚动条显示与隐藏及重新添加或删除事件,主要用于滚动容器高度发生变化时
		if(this.scroll.offsetHeight < this.parent.clientHeight) {  
		    this.scroll.style.right=0;
	...
	setTop:function(t,f){
		...
		if(!f)
		{
			...
			this.scrollHandle.style.top=t+'px';
			// console.log('setTop: ', t/this.scrollBar.clientHeight*100+'%', this.scrollHandle.clientHeight, this.scroll.clientHeight);
		}
		else
		{
	...
	mouseWheel:function(e){
		...
		// 滚动条到边界值时，让默认滚动条继续滚动
		if(flag) {
			if(parseInt(this.scrollHandle.style.top) + this.scrollHandle.clientHeight + this.scrollDown.clientHeight >= this.scrollBar.clientHeight) {
				return true;
			}
		}
		if(!flag) {
			if(parseInt(this.scrollHandle.style.top) - this.scrollUp.clientHeight <= 0) {
				return true;
			}
		}
	...
	extend:function(p){//配置参数
		this.opt={
		  	   ...
		  	   scrollDValue:0  	//滚动的差值,上下的和 半圆部分
			}
		for(var a in p){
			this.opt[a]=p[a];
		}
